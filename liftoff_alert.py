...
# ── 알럿 임계값 ────────────────────────────────────────────────
THRESHOLDS = {
    "ctr_change_pct":     20,   # CTR ±20% 이상 변화
    "cpi_increase_pct":   20,   # CPI +20% 이상 급등
    "install_drop_pct":  -20,   # 인스톨 -20% 이상 급감
}
...
def detect_alerts(today_rows, yest_rows, camp_names, account_name):
    ...
    for t in today_rows:
        ...
        if abs(ctr_chg) >= THRESHOLDS["ctr_change_pct"]:
            alerts.append({
                "account": account_name, "campaign": name, "type": "CTR 이상",
        ...
        if cpi_chg >= THRESHOLDS["cpi_increase_pct"]:
            alerts.append({
                "account": account_name, "campaign": name, "type": "CPI 급등",
        ...
        if install_chg <= THRESHOLDS["install_drop_pct"]:
            alerts.append({
                "account": account_name, "campaign": name, "type": "인스톨 급감",
                "severity": "critical" if install_chg <= -35 else "warning",
                "detail": f"인스톨 ▼ {abs(install_chg):.1f}% ({int(y['installs'])} → {int(t['installs'])})"
            })
    return alerts
...
def send_slack(alerts, account_count):
    if not alerts:
        ...
        for a in alerts:
            ...
            lines.append(f"{icon} *[{a['type']}]* `{a['account']}` - {a['campaign']}\n   └ {a['detail']}")
        msg = (
            f"🚨 *Liftoff Campaign Alert* | {datetime.now().strftime('%Y-%m-%d %H:%M')} KST\n\n"
            + "\n".join(lines)
            + f"\n\n> 🔴 긴급 {len(critical)}개  🟡 주의 {len(warning)}개 | {account_count}개 어카운트 점검 완료"
        )
